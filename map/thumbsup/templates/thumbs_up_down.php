...
<form  data-role="none" method="post" id="thumbsup_<?php echo $item->id ?>" class="thumbsup <?php echo $template ?> <?php if ($item->closed) echo 'closed' ?> <?php if ($item->user_voted) echo 'user_voted' ?> <?php if ($item->closed OR $item->user_voted) echo 'disabled' ?> <?php echo $options->align ?>" name="<?php echo $template ?>">
	...
	<input  data-role="none" class="up"   name="thumbsup_vote" type="submit" value="+1" title="Vote up"   <?php if ($item->closed OR $item->user_voted) echo 'disabled="disabled"' ?> />
	<input  data-role="none" class="down" name="thumbsup_vote" type="submit" value="-1" title="Vote down" <?php if ($item->closed OR $item->user_voted) echo 'disabled="disabled"' ?> />
</form>
